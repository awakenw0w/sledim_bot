...
async def _sync_wall_posts(vk_id: int) -> tuple[list[dict], int | None]:
...
async def _check_online_and_notify(bot: Bot) -> None:
    ...
    users_data = await vk_api.get_users_online_presence(vk_ids)
    if users_data is None:
        logger.warning("VK API не ответил, пропускаем цикл проверки онлайна")
        return
    ...
    for vk_id in vk_ids:
        ...
        if user is None:
            logger.warning("VK ID %s не найден в ответе API", vk_id)
            continue
        ...
        old_status = await db.get_last_status(vk_id)
        ...
        for chat_id in related_chat_ids:
            try:
                ...
                logger.error(
                    "Не удалось синхронизировать сессию chat_id=%s, vk_id=%s: %s",
                    chat_id, vk_id, exc,
                )
...
async def _check_profile_and_notify(bot: Bot) -> None:
    ...
    async def _get_change_settings(chat_id: int) -> dict[str, bool]:
        if chat_id not in change_settings_cache:
            change_settings_cache[chat_id] = await db.get_change_notification_settings(chat_id)
        return change_settings_cache[chat_id]

    for vk_id in vk_ids:
        user = users_map.get(vk_id)
        if user is None:
            logger.warning("VK ID %s не найден в ответе API профиля", vk_id)
            continue

        profile_snapshot = vk_api.extract_profile_snapshot(user)
        old_profile_raw = await db.get_profile_cache(vk_id)
